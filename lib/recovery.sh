...
${output}"
    ...
    if printf '%s\n' "$combined" |
        grep -Eiq \
        'ubuntu.*not installed|ubuntu.*rootfs|distribution.*not found|container.*is busy|is already running'; then
...
recovery_ubuntu() {
    ...
    # Check if container is busy (another session is active)
    if printf '%s\n' "${RECOVERY_LAST_OUTPUT:-}" |
        grep -Eiq 'container.*is busy|is already running'; then

        recovery_warning \
            "Ubuntu container is busy. Waiting for it to become available..."
        ...
        if declare -F system_is_ubuntu_installed >/dev/null 2>&1; then
            ...
            if system_is_ubuntu_installed; then
                ...
                recovery_message \
                    "Ubuntu installation exists. Container was busy but installation is present."
